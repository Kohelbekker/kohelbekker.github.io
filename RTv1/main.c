...
t_vector	color(t_ray *ray, hitable)
{
	float		t;
	t_vector	first;
	t_vector	second;
...
int		hitable_list(t_ray *ray, float t_min, float t_max, t_hit &rec)
{
	t_hit	tmp_rec;
	int		hit_anything = 0;
	double	closest_so_far = t_max;
	while (i < rec->list_size)
	{
		if (rec->list[i]->hit(ray, t_min, closest_so_far, tmp_rec))
		{
			hit_anything = 1;
...
int		shpere(t_ray *ray, float t_min, float t_max, t_hit &rec)
{
	t_vector	oc;
	float		a;
	float		b;
