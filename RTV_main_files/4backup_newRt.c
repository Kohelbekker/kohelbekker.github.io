...
void		init(t_sdl	*sdl)
{
	...
	sdl->lights[0].intensity = 0.2;

	sdl->lights[1].type = 1; //POINT
	sdl->lights[1].intensity = 0.6;
	sdl->lights[1].position = (t_vector){2, 1, 0};

	sdl->lights[2].type = 2; //DIRECTIONAL
	sdl->lights[2].intensity = 0.2;
	sdl->lights[2].position = (t_vector){1, 4, 4};
	/////////////////////////////////////////////////////////////////
}
...
double		*hit_cylinder(t_ray *ray, t_shape shape)
{
	...
	xs = (double *)malloc(sizeof(double) * 2);
	oc = ray->a - shape.center;									// for cylinder its not "center" but "start"
	a = dot(ray->b, ray->b) - pow(dot(ray->b, shape.axis_v), 2);
	b = 2 * (dot(ray->b, oc) -  (dot(ray->b, oc) * dot(oc, shape.axis_v)));
...
double		*hit_cone(t_ray *ray, t_shape shape)
{
	...
	// printf ("e.cos = %f, e.sin = %f, angle = %f\n", sin(shape.angle), cos(shape.angle), shape.angle);
...
t_shape		ClosestIntersaction(t_sdl *sdl, t_ray *ray, double min_t, double max_t)
{
	...
	while (i < 5)
	{
		if (sdl->shapes[i].type == 1 || sdl->shapes[i].type == 3 || sdl->shapes[i].type == 4)
		{
			if (sdl->shapes[i].type == 1)
				ts = hit_sphere(ray, sdl->shapes[i]);
			else if (sdl->shapes[i].type == 3)
				ts = hit_cylinder(ray, sdl->shapes[i]);
			else
				ts = hit_cone(ray, sdl->shapes[i]);
			if (ts[0] < closest_t && min_t < ts[0] && ts[0] < max_t)
			{
				closest_t = ts[0];
				closest_shape = sdl->shapes[i];
				hit = 1;
			}
		...
		else if (sdl->shapes[i].type == 2)
		{
			a = hit_plane(ray, sdl->shapes[i]);  
...
int			main(void)
{
	...
	while (i < WDTH / 2)
	{
		...
		while (j < HGHT/2)
		{
			u = i * sdl->viewport_size / (double)WDTH;
			v = j * sdl->viewport_size / (double)HGHT;
			sdl->ray->b = unit_vector((t_vector){u, v, sdl->projection_plane_z});
